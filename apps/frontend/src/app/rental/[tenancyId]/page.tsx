...
export default function TenantFlexPayPage() {
  ...
  useEffect(() => { load(); }, [tenancyId]); // eslint-disable-line
  ...
  // ── Safe vault values ──────────────────────────────────────────────────────
  const vaultBalance    = safeNum(vault?.vault_balance);
  const targetAmount    = safeNum(vault?.target_amount);
  const installment     = safeNum(vault?.installment_amount);
  const fundedPct       = safeNum(vault?.funded_pct);
  const contribCount    = safeNum(vault?.contribution_count);
  const fundedPeriods   = safeNum(vault?.funded_periods);

  // Quick-pay preset amounts — safe even if installment is 0
  const presetAmounts = installment > 0
    ? [installment, installment * 2, Math.ceil(targetAmount / 4)].filter(a => a > 0)
    : [];
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { key: 'LUMP_SUM', label: 'Lump Sum', desc: 'Landlord gets full amount once vault is funded' },
                  { key: 'DRAWDOWN', label: 'Drawdown', desc: 'Landlord receives monthly from the vault' },
                ].map(opt => (
                  <button key={opt.key} type="button" onClick={() => setCashoutMode(opt.key)}
                    className={`p-4 rounded-2xl border text-left transition-all ${cashoutMode === opt.key ? 'border-teal-500 bg-teal-500/5' : 'border-zinc-800 bg-zinc-900/20'}`}>
                    <p className="font-black text-xs uppercase">{opt.label}</p>
                    <p className="text-[9px] text-zinc-500 mt-1">{opt.desc}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-6 rounded-2xl border border-teal-500/30 bg-teal-500/5 space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="font-black text-xl mt-1">{vault.unit_name ?? 'Your Unit'}</p>
                </div>
              ...
              <div>
                <div className="flex justify-between text-[9px] text-zinc-500 mb-2">
                  ...
                  <span className="text-teal-400 font-bold">{fundedPct}%</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-zinc-900/50 rounded-xl">
                  ...
                  <p className="font-mono font-bold text-lg text-teal-400">₦{fmt(vaultBalance)}</p>
                </div>
                <div className="p-3 bg-zinc-900/50 rounded-xl">
                  <p className="text-[8px] text-zinc-600 uppercase font-bold mb-1">Target</p>
                  <p className="font-mono font-bold text-lg">₦{fmt(targetAmount)}</p>
                </div>
                <div className="p-3 bg-zinc-900/50 rounded-xl">
                ...
                <div className="p-3 bg-zinc-900/50 rounded-xl">
                  ...
                  <p className="font-mono font-bold text-sm text-amber-400">₦{fmt(installment)}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-[9px] text-zinc-500">
                ...
                <span className="text-white font-bold">
                  {vault.next_due_date ? new Date(vault.next_due_date).toLocaleDateString() : '—'}
                </span>
                <span className="ml-auto">{contribCount} contributions · {fundedPeriods} periods funded</span>
              </div>
            </div>
            ...
            {vault.status === 'ACTIVE' && (
              <div className="p-5 rounded-2xl border border-zinc-800 bg-zinc-900/20 space-y-4">
                ...
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      placeholder={installment > 0 ? installment.toString() : '0'}
                ...
                {/* Preset amount buttons — only shown if installment is valid */}
