 ...
 *
 * MODES:
 * 1. TENANT INVITE FLOW (via URL params)
 * 2. STANDARD FLOW (manual role selection)
 */

import { useState, useEffect, Suspense } from 'react';
import { useSearchParams, useRouter } from 'next/navigation';
import Link from 'next/link';
import { Loader2, AlertCircle, ShieldCheck, Home, Info } from 'lucide-react';
...
const ROLES = [
  ...
  },
  {
    ...
    desc: 'Access your rental dashboard, pay rent via Flex-Pay, and manage tenancies.',
    dbRole: 'TENANT',
    accountType: 'TENANT',
  },
  {
  ...
  useEffect(() => {
    const validateInvite = async () => {
      ...
      try {
        ...
        setError(e?.response?.data?.error || 'This tenant invite is invalid or expired.');
      } finally {
          ...
          <div className="space-y-3">
            ...
            {ROLES.map(role => (
              ...
              >
                ...
                <p className="text-[9px] text-teal-500 uppercase font-bold tracking-widest mt-1">{role.sub}</p>
                <p className="text-zinc-500 text-[10px] mt-1">{role.desc}</p>
              </button>
            ))}
          </div>
        )}
