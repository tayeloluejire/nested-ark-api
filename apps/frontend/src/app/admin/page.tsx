...
import {
  Activity, Users, Briefcase, ShieldCheck, DollarSign,
        ...
        <div className="space-y-3">
          {s.ready_for_release > 0 && (
            <Link href="/admin/approval"
              className="flex items-center justify-between p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 hover:bg-emerald-500/10 transition-all">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="text-emerald-500 flex-shrink-0" size={18} />
                <div>
                  <p className="text-emerald-400 text-sm font-bold">
                    {s.ready_for_release} milestone{s.ready_for_release !== 1 ? 's' : ''} ready for release
                  </p>
          ...
          {s.pending_approvals > 0 && (
            <Link href="/admin/approval"
              className="flex items-center justify-between p-4 rounded-xl border border-amber-500/30 bg-amber-500/5 hover:bg-amber-500/10 transition-all">
              <div className="flex items-center gap-3">
                <AlertTriangle className="text-amber-400 flex-shrink-0" size={18} />
                <div>
                  <p className="text-amber-400 text-sm font-bold">
                    {s.pending_approvals} milestone{s.pending_approvals !== 1 ? 's' : ''} awaiting verification
                  </p>
            ...
            {[
              {
                label: 'Total Users',
                value: s.users.total,
                sub: `${s.users.govts}G · ${s.users.contractors}C · ${s.users.investors}I`,
                icon: Users, color: 'text-teal-500', href: '/admin/users',
              },
              {
                label: 'Active Projects',
                value: s.projects.active,
                sub: `${s.projects.verified} verified · ${s.projects.total} total`,
                icon: Briefcase, color: 'text-emerald-500', href: '/admin/projects',
              },
              {
                label: 'Total Invested',
                value: `$${(safeN(s.investments?.total) / 1_000_000).toFixed(2)}M`,
                sub: `${s.investments.count} commitments`,
                icon: TrendingUp, color: 'text-amber-400', href: '/admin/projects',
              },
              {
                label: 'Ledger Events',
                value: s.ledger_events,
                sub: 'Immutable records',
                icon: Database, color: 'text-teal-500', href: '/admin/ledger',
              },
            ].map(card => {
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              { label: 'Paid Out', value: s.milestones.paid, icon: CheckCircle2, color: 'text-teal-500' },
              {
                ...
                value: `$${(safeN(s.milestones?.total_value) / 1_000_000).toFixed(2)}M`,
          ...
          {s.recent_activity?.length > 0 && (
            <div>
                  ...
                  <div key={i}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-bold uppercase tracking-widest text-teal-400">
                        {String(ev.transaction_type ?? '').replace(/_/g, ' ')}
                      </span>
                    </div>
                    {/* ✅ FIXED: was `new DatesafeF(...)` — now correct Date formatting */}
              ...
              <Link href="/admin/ledger"
                className="mt-3 block text-center text-teal-500 text-xs uppercase font-bold tracking-widest hover:underline">
                View Full Ledger →
              </Link>
            </div>
          )}
