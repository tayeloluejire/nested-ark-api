 ...
 *   Units · Tenants · Litigation · Receipts · Milestones
 *
 * FIXED in this version:
 *   • All hooks hoisted to top level inside RentalManagementContent — React #321 resolved
 *   • Stray useState/handleSaveUnit declarations removed from module scope
 *   • useSearchParams wrapped in <Suspense> as required by Next.js App Router
 *   • All API calls use `api` wrapper (Render backend) — 404s resolved
 *   • Promise.allSettled so one failed endpoint can't crash the page
 *   • useRouter back-navigation added
 *   • Milestones tab added (preserved from page_manag.tsx)
 *   • All original features retained: UnitEditModal, NoticeModal, InviteButton,
 *     photo strip, amenities, vault progress bar, litigation notices, receipts
 */
...
interface Tenancy {
  ...
  rent_amount: number; currency?: string; start_date: string; end_date?: string;
...
interface Milestone {
  ...
  amount_ngn: number; due_date: string;
  contractor_name?: string; completion_pct: number;
}
...
const MILESTONE_STATUS_STYLES: Record<string, string> = {
  ...
  COMPLETED:    'border-teal-500/40 text-teal-500',
      ...
      <div className="w-full max-w-2xl bg-zinc-950 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          ...
          {activeSection === 'rates' && (
            <div className="space-y-4">
              ...
              {[
                { key: 'rent_amount',      label: 'Monthly Rent *',         help: 'Used for Flex-Pay vault installment calculations' },
                { key: 'service_charge',   label: 'Service Charge / Month', help: 'Estate maintenance, security, waste etc.' },
                { key: 'security_deposit', label: 'Security Deposit',       help: 'Refundable deposit collected on move-in' },
              ].map(f => (
              ...
              <div className="p-4 rounded-2xl border border-teal-500/20 bg-teal-500/5 space-y-2">
                ...
                {[['Rent', form.rent_amount], ['Service Charge', form.service_charge]].map(([label, val]) => (
        ...
        <div className="p-5 border-t border-zinc-900 flex gap-2 flex-shrink-0">
          <button onClick={onClose}
            className="flex-1 py-3 rounded-xl border border-zinc-800 text-zinc-500 font-bold text-[10px] uppercase tracking-widest hover:border-zinc-600 hover:text-zinc-300 transition-all">
            Cancel
          </button>
...
function NoticeModal({ tenancies, onClose, onSuccess }: { tenancies: Tenancy[]; onClose: () => void; onSuccess: () => void }) {
  ...
  const send = async () => {
    ...
    try {
      await api.post('/api/notices/generate', { tenancy_id: tenancyId, notice_type: noticeType, notes });
...
// ── AddUnitModal (quick-register shell — full detail via UnitEditModal) ────────
function AddUnitModal({ projectId, onClose, onSaved }: { projectId: string; onClose: () => void; onSaved: () => void }) {
  const [saving, setSaving] = useState(false);
  const [err,    setErr]    = useState('');
  const [form,   setForm]   = useState({
    unit_name: '', unit_type: 'APARTMENT', bedrooms: '1', bathrooms: '1',
    floor_area_sqm: '', rent_amount: '', currency: 'NGN', status: 'VACANT',
  });
  const set = (k: string, v: string) => setForm(f => ({ ...f, [k]: v }));

  const handleSave = async () => {
    if (!form.unit_name.trim() || !form.rent_amount) { setErr('Unit name and rent are required.'); return; }
    setSaving(true); setErr('');
    try {
      await api.post(`/api/rental/project/${projectId}/units`, {
      ...
      setErr(ex?.response?.data?.error ?? 'Failed to register unit.');
    } finally { setSaving(false); }
  };

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-3xl w-full max-w-md space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[8px] text-teal-500 uppercase font-black tracking-widest">Register Unit</p>
            <h2 className="text-lg font-black uppercase tracking-tight">New Unit</h2>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl text-zinc-600 hover:text-white hover:bg-zinc-800 transition-all">
            <X size={16} />
          </button>
        ...
        {err && <div className="p-3 rounded-xl bg-red-500/5 border border-red-500/20 text-red-400 text-xs font-bold">{err}</div>}

        <div className="space-y-3">
          <div>
            <label className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-1 block">Unit Name *</label>
            <input name="unitName" value={form.unit_name} onChange={e => set('unit_name', e.target.value)}
              placeholder="e.g. Flat 101, Unit B, Room 3" required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-1 block">Type</label>
              <select value={form.unit_type} onChange={e => set('unit_type', e.target.value)}
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-teal-500 outline-none">
                {UNIT_TYPES.map(t => <option key={t} value={t}>{t.replace(/_/g, ' ')}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-1 block">Status</label>
              <select value={form.status} onChange={e => set('status', e.target.value)}
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-teal-500 outline-none">
                {['VACANT', 'OCCUPIED', 'MAINTENANCE', 'PENDING'].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-1 block">Bedrooms</label>
              <input type="number" min={0} value={form.bedrooms} onChange={e => set('bedrooms', e.target.value)}
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-teal-500 outline-none" />
            </div>
            <div>
              <label className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-1 block">Bathrooms</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-1 block">Currency</label>
            ...
            <div>
              <label className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-1 block">Monthly Rent *</label>
              <input type="number" min={0} value={form.rent_amount} onChange={e => set('rent_amount', e.target.value)}
                placeholder="e.g. 350000"
                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white placeholder:text-zinc-600 focus:border-teal-500 outline-none" />
            </div>
          </div>
          <p className="text-[9px] text-zinc-600">After registering, click <span className="text-teal-500 font-bold">Edit</span> on the unit card to add photos, amenities and full details.</p>
        </div>

        <div className="flex gap-3 pt-1">
          <button onClick={onClose}
            className="flex-1 px-4 py-3 border border-zinc-800 rounded-xl text-zinc-500 font-bold text-xs uppercase tracking-widest hover:border-zinc-600 hover:text-zinc-300 transition-all">
            Cancel
          </button>
          <button onClick={handleSave} disabled={saving}
            className="flex-1 px-4 py-3 bg-teal-500 text-black rounded-xl font-black text-xs uppercase tracking-widest hover:bg-teal-400 transition-all disabled:opacity-60 flex items-center justify-center gap-2">
...
// ── Main content — ALL hooks at top level, Suspense handles useSearchParams ───
function RentalManagementContent() {
  ...
  // ── ALL useState declarations MUST remain here, before any conditional ───
  const [tab,         setTab]         = useState<'units' | 'tenants' | 'litigation' | 'receipts' | 'milestones'>(
    ['units', 'tenants', 'litigation', 'receipts', 'milestones'].includes(tabParam) ? tabParam : 'units'
  );
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true); setError('');
    try {
      // allSettled: one 404 won't crash the whole page
      const [uRes, tRes, rRes, sRes, mRes] = await Promise.allSettled([
        api.get(`/api/rental/units/${projectId}`),
        api.get(`/api/rental/project/${projectId}`).then(r => ({data: {tenancies: r.data.tenancies}})),
        api.get(`/api/rental/project/${projectId}/receipts`),
        api.get(`/api/rental/project/${projectId}/summary`),
        api.get('/api/milestones', { params: { project_id: projectId } }),
      ]);

      if (uRes.status === 'fulfilled') setUnits(uRes.value.data.units ?? []);
      if (tRes.status === 'fulfilled') setTenancies(tRes.value.data.tenancies ?? []);
      if (rRes.status === 'fulfilled') setReceipts(rRes.value.data.receipts ?? []);
      if (sRes.status === 'fulfilled') setSummary(sRes.value.data.summary ?? null);
      if (mRes.status === 'fulfilled') {
        const d = mRes.value.data;
        setMilestones(d?.milestones ?? d ?? []);
      }
  ...
  // ── Fallback summary computed from raw data ───────────────────────────────
  const computedSummary: Summary = summary ?? {
    total_units:          units.length,
    occupied:             units.filter(u => u.status === 'OCCUPIED').length,
    vacant:               units.filter(u => u.status === 'VACANT').length,
    monthly_rent_roll:    units.filter(u => u.status === 'OCCUPIED').reduce((s, u) => s + safeN(u.rent_amount), 0),
    vault_total:          tenancies.reduce((s, t) => s + safeN(t.vault_balance), 0),
    overdue_count:        overdueCount,
    collected_this_month: receipts.filter(r => {
      const d = new Date(r.paid_at);
      const now = new Date();
      return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
    }).reduce((s, r) => s + safeN(r.amount_ngn), 0),
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      {addingUnit   && <AddUnitModal projectId={projectId!} onClose={() => setAddingUnit(false)} onSaved={load} />}
      ...
      <div className="border-b border-zinc-800 bg-black px-6 py-2 flex items-center gap-5 text-[9px] font-mono uppercase tracking-widest overflow-x-auto">
        <span className="flex items-center gap-1.5 text-teal-500 flex-shrink-0"><Building2 size={8} /> Rental Command Centre</span>
        {lastSync && <span className="text-zinc-600 flex-shrink-0">Synced {lastSync}</span>}
        <span className="text-zinc-400 flex-shrink-0">{units.length} unit{units.length !== 1 ? 's' : ''}</span>
        {overdueCount > 0 && (
          <span className="flex items-center gap-1 text-red-400 font-black flex-shrink-0">
            <AlertCircle size={8} /> {overdueCount} overdue
          </span>
        )}
      </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="border-l-2 border-teal-500 pl-5">
            ...
            <p className="text-zinc-500 text-xs mt-1">Manage units, tenants, notices, income and milestones for this property</p>
          </div>
        ...
        {error && (
          <div className="p-4 rounded-xl bg-red-500/5 border border-red-500/20 text-red-400 flex items-center gap-2 text-sm font-bold">
            ...
            <button onClick={load} className="ml-auto text-teal-500 text-xs uppercase font-black tracking-widest hover:text-white transition-colors">Retry →</button>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total Units',          value: computedSummary.total_units,                                              color: 'text-white',    sub: `${computedSummary.occupied} occupied · ${computedSummary.vacant} vacant` },
            { label: 'Monthly Rent Roll',    value: `₦${safeF(computedSummary.monthly_rent_roll)}`,                          color: 'text-teal-400', sub: 'expected per month' },
            { label: 'Collected This Month', value: `₦${safeF(computedSummary.collected_this_month)}`,                      color: 'text-amber-400',sub: 'received in vaults' },
            { label: 'Overdue Tenants',      value: computedSummary.overdue_count,                                           color: computedSummary.overdue_count > 0 ? 'text-red-400' : 'text-teal-400', sub: computedSummary.overdue_count > 0 ? 'action required' : 'all current' },
          ].map(s => (
        ...
        {overdueCount > 0 && (
          <div className="p-4 rounded-2xl border border-red-500/30 bg-red-500/5 flex items-center justify-between gap-4 flex-wrap">
            ...
            <button onClick={() => { setNoticeModal(true); handleTabChange('litigation'); }}
        ...
        <div className="flex items-center gap-1 p-1 bg-zinc-900/50 border border-zinc-800 rounded-2xl w-fit flex-wrap">
          {([
            { key: 'units',       label: 'Units & Property', icon: LayoutGrid, badge: units.length,       badgeRed: false },
            { key: 'tenants',     label: 'Tenants',          icon: Users,      badge: tenancies.length,   badgeRed: false },
            { key: 'litigation',  label: 'Litigation',       icon: Gavel,      badge: overdueCount,       badgeRed: true  },
            { key: 'receipts',    label: 'Receipts',         icon: Receipt,    badge: receipts.length,    badgeRed: false },
            { key: 'milestones',  label: 'Milestones',       icon: BarChart3,  badge: milestones.length,  badgeRed: false },
          ] as const).map(t => {
            const Icon = t.icon;
            ...
            return (
              <button key={t.key} onClick={() => handleTabChange(t.key)}
                ...
                {t.badge > 0 && (
                  <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-black ${t.badgeRed ? 'bg-red-500 text-white' : isActive ? 'bg-black/20 text-black' : 'bg-zinc-800 text-zinc-400'}`}>
                    {t.badge}
                  </span>
                )}
              </button>
          ...
          <div className="space-y-5">
            ...
            {units.length === 0 ? (
              <div className="py-20 text-center border border-dashed border-zinc-800 rounded-2xl space-y-4">
                ...
                <p className="text-zinc-600 text-sm">Register units to start setting rents and inviting tenants.</p>
                <button onClick={() => setAddingUnit(true)}
                  ...
                  <Plus size={11} /> Register First Unit
                </button>
              </div>
            ) : (
              <div className="space-y-4">
        ...
        {tab === 'tenants' && (
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => handleTabChange('units')}
            ...
            {tenancies.length === 0 ? (
              ...
              <div className="space-y-3">
                {tenancies.map(t => {
                  ...
                  } as Record<string, string>)[t.status] ?? 'border-zinc-800 bg-zinc-900/10';
                    ...
                    <div key={t.id} className={`p-5 rounded-2xl border ${statusCfg} space-y-4`}>
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-500">{t.unit_name} · {t.currency ?? 'NGN'} {safeF(t.rent_amount)}/mo</p>
                          <div className="flex items-center gap-3 mt-2 flex-wrap">
                            ...
                            {t.end_date && <span className="flex items-center gap-1 text-[9px] text-zinc-600">Until {fmtDate(t.end_date)}</span>}
                          </div>
                        </div>
                ...
                {overdueTenancies.map(t => (
                  <div key={t.id} className="p-4 rounded-2xl border border-red-500/20 bg-red-500/5 flex items-center justify-between gap-4 flex-wrap">
                    <div>
                      ...
                      <p className="text-[9px] text-zinc-500">{t.unit_name} · {t.days_overdue} days overdue · {t.currency ?? 'NGN'} {safeF(t.rent_amount)} due</p>
                    </div>
                    <button onClick={() => setNoticeModal(true)}
        ...
        {tab === 'milestones' && (
          <div className="space-y-5">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <p className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest">
                {milestones.length} milestone{milestones.length !== 1 ? 's' : ''} on record
              </p>
              <Link href={`/projects/${projectId}/milestones/add`}
                className="flex items-center gap-1.5 px-4 py-2 bg-teal-500 text-black font-black text-[9px] uppercase rounded-xl hover:bg-white transition-all">
            ...
            {milestones.length === 0 ? (
              <div className="py-20 text-center border border-dashed border-zinc-800 rounded-2xl space-y-5">
                <BarChart3 className="text-zinc-700 mx-auto" size={40} />
                <div>
                  <p className="text-zinc-400 font-bold">No milestones yet</p>
                  <p className="text-zinc-600 text-sm mt-1">Add project milestones to track construction or renovation progress</p>
                </div>
                <Link href={`/projects/${projectId}/milestones/add`}
                  className="inline-flex items-center gap-2 px-6 py-3 bg-teal-500 text-black font-bold text-xs uppercase tracking-widest rounded-xl hover:bg-white transition-all">
              ...
              <div className="space-y-3">
                {milestones.map(m => (
                  <div key={m.id} className="p-5 rounded-2xl border border-zinc-800 bg-zinc-900/20 hover:border-zinc-700 transition-all space-y-3">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <span className={`text-[8px] px-2 py-0.5 rounded border font-bold uppercase ${MILESTONE_STATUS_STYLES[m.status] ?? 'border-zinc-700 text-zinc-500'}`}>
                            {m.status.replace('_', ' ')}
                          </span>
                          {m.contractor_name && (
                            <span className="text-[8px] text-zinc-600 border border-zinc-800 px-2 py-0.5 rounded">{m.contractor_name}</span>
                          )}
                        </div>
                        <h3 className="font-bold text-base">{m.title}</h3>
                        {m.due_date && (
                          <p className="text-[9px] text-zinc-500 flex items-center gap-1 mt-1"><Calendar size={8} /> Due: {fmtDate(m.due_date)}</p>
                        )}
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-mono font-bold text-lg">₦{safeF(m.amount_ngn)}</p>
                        <p className="text-[8px] text-zinc-600 uppercase font-bold">contract value</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[9px] font-bold uppercase">
                        ...
                        <span className="text-teal-500">{safeD(m.completion_pct, 0)}%</span>
                      </div>
                      <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden">
                        <div className="h-full bg-teal-500 transition-all duration-700"
                          style={{ width: `${Math.min(safeN(m.completion_pct), 100)}%` }} />
                      </div>
                    </div>

                    <div className="flex items-center gap-2 text-[9px]">
                      {m.status === 'COMPLETED'   && <CheckCircle2 size={10} className="text-teal-500" />}
                      {m.status === 'DISPUTED'    && <XCircle size={10} className="text-red-400" />}
                      {m.status === 'IN_PROGRESS' && <Clock size={10} className="text-blue-400" />}
                      {m.status === 'PENDING'     && <Clock size={10} className="text-zinc-600" />}
                      <span className="text-zinc-600">
                        {m.status === 'COMPLETED'   && 'Milestone completed and verified'}
                        {m.status === 'DISPUTED'    && 'Under dispute — review required'}
                        {m.status === 'IN_PROGRESS' && 'Currently in progress'}
                        {m.status === 'PENDING'     && 'Awaiting start'}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        {/* Trust footer */}
        <div className="pt-4 flex flex-wrap gap-3 border-t border-zinc-900">
          {[
            <><ShieldCheck size={9} /> Tri-Layer Verified</>,
            <><TrendingUp size={9} /> Yield Engine Active</>,
            <><CheckCircle2 size={9} /> Immutable Ledger</>,
            <><Receipt size={9} /> Court-Admissible Receipts</>,
          ].map((badge, i) => (
            <span key={i}
              className="flex items-center gap-1 text-[8px] text-teal-500 border border-teal-500/20 bg-teal-500/5 px-3 py-1.5 rounded-lg font-bold uppercase tracking-widest">
              {badge}
            </span>
          ))}
        </div>
