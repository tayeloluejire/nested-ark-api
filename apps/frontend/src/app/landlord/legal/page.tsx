...
/**
 * /landlord/legal/page.tsx  (Litigation Command Centre)
 *
 * Tenant dropdown calls GET /api/landlord/tenancies/active
 * — cross-project, landlord-scoped, no projectId required.
 *
 * Notice issue calls POST /api/notices/generate
 * — requires tenancy_id, notice_type, optional amount_overdue + notes.
 */
...
import {
  ...
  CheckCircle, ChevronDown, Hash, Building2,
} from 'lucide-react';
...
export default function LitigationCommandPage() {
  ...
  // Notice history (all landlord notices)
  ...
  useEffect(() => {
    ...
    api.get('/api/landlord/tenancies/active')
      .then(r => setTenancies(r.data.tenancies ?? []))
      .catch(() => setTenErr('Could not load tenants. Check your connection.'))
      .finally(() => setLoadingTen(false));
  }, [authLoading]);

  // ── Load all notices across landlord's portfolio ─────────────────────────
  useEffect(() => {
    if (authLoading) return;
    // Use the management endpoint on the first project or a landlord-wide notices endpoint
    api.get('/api/landlord/notices')
      .then(r => setNotices(r.data.notices ?? []))
      .catch(() => setNotices([]))   // silently degrade — table still shows "no notices"
      .finally(() => setLoadingNot(false));
  }, [authLoading]);

  // ── Submit notice ─────────────────────────────────────────────────────────
  const handleIssue = async () => {
    if (!selTenancy || !noticeType) {
      setSubmitErr('Please select a tenant and notice type.');
      return;
    }
    setSubmitting(true);
    setSubmitErr('');
    try {
      ...
      const issued = r.data;
      // Build a Notice object from the response to show in the confirmation card
      setSubmitted({
        id:                issued.notice_id,
        notice_number:     issued.notice_number,
        notice_type:       noticeType,
        tenant_name:       tenancies.find(t => t.tenancy_id === selTenancy)?.tenant_name ?? '',
        unit_name:         tenancies.find(t => t.tenancy_id === selTenancy)?.unit_name ?? '',
      ...
      setSubmitErr(e?.response?.data?.error ?? 'Failed to issue notice. Try again.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  // Unique project count from tenancies
  const projectCount  = new Set(tenancies.map(t => t.project_number)).size;
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-10 w-full space-y-8">
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#0a0a0a] border border-zinc-800 rounded-3xl p-7 w-full max-w-lg space-y-5 shadow-2xl">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowForm(false)} className="p-2 rounded-lg border border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600 transition-all">
                  <X size={14} />
                </button>
              </div>
              ...
              {selTenancy && (() => {
                ...
                return (
                  <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-900/30 text-xs space-y-1">
                    ...
                    <p className="text-zinc-500">Rent: <span className="text-white font-mono">{t.currency} {safeN(t.rent_amount).toLocaleString()}</span></p>
                    {t.next_due_date && <p className="text-zinc-500">Next due: <span className="text-amber-400">{new Date(t.next_due_date).toLocaleDateString('en-GB')}</span></p>}
                  </div>
                );
              })()}
              ...
              {/* Amount overdue */}
              <div>
                <label className="block text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-2">Amount Overdue (₦) — optional</label>
                <input
                  type="number"
                  value={amountOver}
                  onChange={e => setAmountOver(e.target.value)}
              ...
              {/* Notes */}
              <div>
                <label className="block text-[9px] text-zinc-500 uppercase font-bold tracking-widest mb-2">Additional Notes (optional)</label>
                <textarea
                  rows={2}
                  value={notes}
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleIssue} disabled={submitting || !selTenancy}
                  ...
                  {submitting ? <><Loader2 size={12} className="animate-spin" /> Issuing…</> : <><ShieldAlert size={12} /> Issue Notice</>}
                </button>
              </div>
            </div>
          </div>
        ...
        {/* ── Issued confirmation toast ── */}
        {submitted && (
          <div className="p-5 rounded-2xl border border-teal-500/20 bg-teal-500/5 space-y-3">
            <div className="flex items-center gap-2 text-teal-400">
              <CheckCircle size={16} /> <p className="font-black text-sm uppercase tracking-tight">Notice Issued Successfully</p>
            </div>
          ...
          {loadingNot ? (
            ...
            <div className="space-y-2">
              {notices.map(n => {
                ...
                return (
                  <div key={n.id} className="flex items-center justify-between p-4 rounded-2xl border border-zinc-800 bg-zinc-900/10 hover:border-zinc-700 transition-all">
