...
import {
  ...
  BadgeCheck, Banknote, Zap, Star, Eye, EyeOff,
} from 'lucide-react';
...
export default function LandlordBankPage() {
  ...
  // Auto-resolve account name when account_number + bank_code are set
  const handleBankChange = async (acctNum: string, bCode: string) => {
    ...
    try {
      ...
      setAddError(ex?.response?.data?.error ?? 'Could not verify account number.');
    } finally {
      setResolving(false);
    }
  };
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      setAddOk('Bank account saved & payout recipient created!');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setError(ex?.response?.data?.error ?? 'Could not delete account.');
    }
  };

  const handleRepairRecipient = async (id: string) => {
    try {
      await api.post(`/api/landlord/bank-accounts/${id}/create-recipient`, {});
      loadAccounts();
    } catch (ex: any) {
      setError(ex?.response?.data?.error ?? 'Could not create recipient.');
    }
  };
  ...
  const handlePayout = async () => {
    ...
    try {
      const res = await api.post('/api/landlord/payout', {
        bank_account_id: payoutAcct,
        amount_ngn:      parseFloat(payoutAmt),
      });
      setPayoutMsg(res.data.message ?? `₦${safeF(res.data.net_to_landlord_ngn)} transferred successfully!`);
      setPayoutAmt('');
    } catch (ex: any) {
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-10">
            ...
            {payoutReadyAccts.length > 0
              ? <><ShieldCheck size={14} className="text-teal-400 flex-shrink-0" />
                  <p className="text-teal-400 text-xs font-bold">
                    ...
                    Rent will auto-transfer within 24h of collection.
                  </p></>
              : <><AlertCircle size={14} className="text-amber-400 flex-shrink-0" />
                  <p className="text-amber-400 text-xs font-bold">
                    No accounts are payout-ready. Re-add your bank account to create a Paystack recipient.
                  </p></>
            }
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {accounts.map(acct => (
                <div key={acct.id}
                  ...
                  <div className="flex items-center justify-between">
                    <button
                      onClick={() => setShowNums(n => ({ ...n, [acct.id]: !n[acct.id] }))}
                      className="text-zinc-600 hover:text-zinc-400 transition-colors">
                      {showNums[acct.id] ? <EyeOff size={11} /> : <Eye size={11} />}
                    </button>
                    <div className="flex items-center gap-2">
                      {!acct.payout_ready && (
                        <button onClick={() => handleRepairRecipient(acct.id)}
                          className="text-[8px] text-amber-400 font-bold uppercase tracking-widest hover:text-amber-300 transition-colors">
                          Repair →
                        </button>
                      )}
                      {acct.payout_ready && (
        ...
        {payoutReadyAccts.length > 0 && (
          <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/20 space-y-5">
            <div>
              ...
              <p className="text-xs text-zinc-600">Trigger an on-demand transfer from collected rent balance. 2% platform fee applies.</p>
            </div>
            ...
            {payoutAmt && parseFloat(payoutAmt) > 0 && (
              <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 text-[10px] font-mono text-zinc-400 flex items-center gap-4">
                <span>Gross: <span className="text-white">₦{safeF(parseFloat(payoutAmt))}</span></span>
                <span>Platform fee (2%): <span className="text-amber-400">₦{safeF(Math.round(parseFloat(payoutAmt) * 0.02))}</span></span>
            ...
            <button
              ...
              disabled={payoutBusy || !payoutAcct || !payoutAmt}
        ...
        <div className="p-5 rounded-2xl border border-zinc-800 bg-zinc-900/10 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { step: '01', title: 'Tenant pays',      body: 'Rent is collected via Paystack and held in the Ark platform balance.' },
              { step: '02', title: 'Auto-trigger',     body: 'Within 24h, the cron engine initiates a transfer to your default account.' },
              { step: '03', title: '98% to you',       body: '2% platform fee is deducted. Your net is transferred via Paystack instant.' },
            ].map(s => (
            ...
            <div className="space-y-3">
              ...
              <select
                ...
                onChange={e => {
                  ...
                  setForm(f => ({ ...f, bank_code: newCode, bank_name: newName, account_name: '' }));
                  setResolved(false);
                  if (form.account_number.length === 10 && newCode) {
                    handleBankChange(form.account_number, newCode);
                  }
                }}>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={e => {
                    ...
                    setResolved(false);
                    if (v.length === 10 && form.bank_code) handleBankChange(v, form.bank_code);
                  }}
                />
                {resolving && (
              ...
              <div className={`p-3 rounded-xl border transition-all ${resolved ? 'border-teal-500/30 bg-teal-500/5' : 'border-zinc-800 bg-zinc-900'}`}>
                <p className="text-[8px] text-zinc-500 uppercase font-bold tracking-widest mb-0.5">Account Name</p>
                <p className={`text-sm font-bold ${resolved ? 'text-teal-400' : 'text-zinc-600'}`}>
                  {resolving ? 'Verifying…' : form.account_name || 'Will auto-resolve from Paystack'}
                </p>
              </div>
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <div
                  onClick={() => setForm(f => ({ ...f, set_as_default: !f.set_as_default }))}
