...
import {
  ...
  FileText, ChevronRight, Plus, Download, Filter,
  Mail, Phone
} from 'lucide-react';
...
interface Tenant {
  ...
  status: string; // active | pending | overdue | terminated
  move_in_date?: string;
...
export default function LandlordTenantsPage() {
  ...
  const sendReminder = (tenant: Tenant) => {
    const text = `Hello ${tenant.tenant_name}, this is a friendly reminder from Nested Ark OS that your rent of ${tenant.currency || 'NGN'} ${Number(tenant.rent_amount).toLocaleString()} is due. Please visit your tenant portal to make payment.`;
          ...
          {filtered.map(tenant => (
            ...
            >
                ...
                <div className="flex items-center gap-3 flex-wrap shrink-0">
                  <div className="text-right">
                    ...
                    <p className="text-xs font-mono text-teal-400">
                      {tenant.currency || 'NGN'} {Number(tenant.rent_amount).toLocaleString()} / {tenant.payment_frequency || 'mo'}
                    </p>
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-zinc-800 flex-wrap">
                ...
                <Link
                  href={`/landlord/notices?tenant=${tenant.id}`}
                ...
                <Link
                  href={`/landlord/receipts?tenant=${tenant.id}`}
                  className="flex items-center gap-1.5 text-[10px] font-black uppercase bg-zinc-800 text-zinc-400 border border-zinc-700 px-3 py-2 rounded-xl hover:bg-zinc-700 transition-colors"
                >
                ...
                {tenant.status === 'pending' && (
                  <Link
                    href={`/landlord/onboard/${tenant.unit_id}`}
