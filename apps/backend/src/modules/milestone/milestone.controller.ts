...
export class MilestoneController {
  ...
  async createMilestone(req: Request, res: Response) {
    try {
      ...
      const { title, description, budget_allocation, estimated_start_date, estimated_completion_date, deliverables } = req.body;
  ...
  async submitEvidence(req: Request, res: Response) {
    try {
      const { milestoneId } = req.params;
      const { evidence_type, file_url, description } = req.body;
      if (!evidence_type || !file_url) {
        res.status(400).json({ error: "evidence_type and file_url required" });
        return;
      }
      const contractor = await this.milestoneService.getMilestoneById(milestoneId);
      const evidence = await this.milestoneService.submitEvidence(req.body, milestoneId, contractor.contractor_id || "");
